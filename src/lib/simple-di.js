...
export const registerService = (name, instance) => {
  ...
  if (container[name]) throw new Error(`simple-di: registerService('${name}') - service with this name has been registered already`)
...
export const registerServiceFactory = (name, factory) => {
  ...
  if (container[name]) throw new Error(`simple-di: registerServiceFactory('${name}') - service with this name has been registered already`)
...
export const registerModulesAsync = async (urls) => {
  ...
  modules.reduce((_, { __simpleDiAddServices } = module || {}) => {
